...
<html>
  ...
  <body style="background:url('${zone.zoneStyle}'); -webkit-background-size:cover; background-size:cover;">
	<% 
		Blog blog=(Blog)request.getAttribute("modifyBlog");
		String title=""; 
		
		String content="**A New Amazing Blog**";
		//if(blog!=null){
  ...
  <nav class="navbar navbar-inverse" role="navigation">
	...
	<form action="/AmzBlog/servlet/updataBlog.db?blogId=<%=blog.getBlogId() %>" method="post">
		<input type="text" value="<%=title %>" name="blogTitle2" id="blogTitle" placeholder="输入博客标题" class="form-control" style="display: inline-block; margin:.5em 10%; width:80%">
		<div class="editormd" id="test-editormd">
    		<textarea value="<%=content %>" class="editormd-markdown-textarea" name="blogContent2" id="editormd"></textarea>
    		<!-- 第二个隐藏文本域，用来构造生成的HTML代码，方便表单POST提交，这里的name可以任意取，后台接受时以这个name键为准 -->
    		<!-- html textarea 需要开启配置项 saveHTMLToTextarea == true -->
    		<textarea class="editormd-html-textarea" name="blogContentHtml2" id="editorhtml2"></textarea>       
    	</div>
